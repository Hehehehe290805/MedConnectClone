...
// joined. Missed virtual appointments are cancelled; refund requests go through admin reports.
const checkVirtualNoJoin = async () => {
    ...
    for (const appt of timedOut) {
        ...
        const patientMissed = !appt.patientJoined;
        const providerMissed = !appt.providerJoined;

        appt.status = "cancelled";
        appt.rejectionReason = patientMissed && providerMissed
            ? "Neither party joined the video call within 5 minutes."
            : patientMissed
                ? "Patient did not join the video call within 5 minutes."
                : "Provider did not join the video call within 5 minutes.";
        await appt.save();

        if (patientMissed) {
            notify(appt.patientId, "appointment_cancelled", "Appointment Cancelled - No-Show",
                "Your virtual appointment was cancelled because you did not join within 5 minutes. Your deposit is non-refundable.");
            if (providerId) notify(providerId, "appointment_cancelled", "Patient Missed Appointment",
                "The patient did not join the virtual appointment within 5 minutes. The appointment was cancelled with no refund.");
        } else if (providerMissed) {
            notify(appt.patientId, "appointment_cancelled", "Provider Missed Appointment",
                "The provider did not join the virtual appointment within 5 minutes. You may file a report so an admin can review whether a refund is appropriate.");
            if (providerId) notify(providerId, "appointment_cancelled", "Appointment Cancelled - Provider No-Show",
                "You did not join the virtual appointment within 5 minutes. The patient may file a report for admin review.");
        }
    }
};
